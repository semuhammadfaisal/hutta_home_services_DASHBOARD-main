...
function configuredEncryptionKey() {
  let configured = String(process.env.TAX_ID_ENCRYPTION_KEY || '').trim();
  if (!configured && process.env.NODE_ENV !== 'production') {
    const loaded = require('dotenv').config({ path: path.join(__dirname, '../.env') });
    configured = String(process.env.TAX_ID_ENCRYPTION_KEY || loaded.parsed?.TAX_ID_ENCRYPTION_KEY || '').trim();
  }
  return configured;
}
