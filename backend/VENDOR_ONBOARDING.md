...
- `EMAIL_REPLY_TO`: exactly `sales@smplfix.com`, which must be a monitored Google Workspace mailbox or alias.
- `RESEND_API_KEY`: a production sending key from the Resend account where `smplfix.com` is verified.
- `PUBLIC_APP_URL` and `FRONTEND_URL`: `https://hutta-home-services-dashboard-main.onrender.com` without a trailing path.

Production refuses localhost, private-network, non-HTTPS, and path-bearing public application URLs. It also refuses to start unless Resend and the exact `sales@smplfix.com` sender and reply-to identity are configured. There is no Gmail SMTP fallback, so software email cannot silently leave from another account. Keep API keys only in Render environment variables; never place them in this repository or documentation.
...
1. Confirm `smplfix.com` is verified in Resend and `sales@smplfix.com` can receive replies in Google Workspace.
...
8. Confirm the received message shows `sales@smplfix.com` for both From and Reply-To, and that SPF, DKIM, and DMARC pass.
