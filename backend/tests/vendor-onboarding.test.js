...
test('email configuration requires Resend with the sales@huttas.com sender identity', () => {
  const previous = {
  ...
  try {
    ...
    assert.equal(getEmailDeliveryStatus().replyTo, 'sales@huttas.com');

    process.env.EMAIL_FROM = 'Hutta Home Services <another@huttas.com>';
    assert.equal(getEmailDeliveryStatus().provider, 'unconfigured');
    assert.match(getEmailDeliveryStatus().warning, /sales@huttas\.com/);

    process.env.EMAIL_FROM = 'Hutta Home Services <sales@huttas.com>';
    process.env.EMAIL_REPLY_TO = 'another@huttas.com';
    assert.equal(getEmailDeliveryStatus().provider, 'unconfigured');
  } finally {
    for (const [key, value] of Object.entries(previous)) {
      if (value === undefined) delete process.env[key]; else process.env[key] = value;
...
test('email templates use the clean white Hutta corporate shell', () => {
  ...
  assert.match(emailSource, /mailto:sales@huttas\.com/);
