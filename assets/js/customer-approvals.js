(() => {
  ...
  function renderList() {
    ...
    const priority = { changes_requested: 0, pending: 1, approved: 2 };
    const sorted = [...approvals].sort((a, b) => {
      const statusDifference = (priority[decisionStatus(a)] ?? 3) - (priority[decisionStatus(b)] ?? 3);
      return statusDifference || new Date(b.decision?.decisionAt || b.outgoingQuote?.sentAt || 0) - new Date(a.decision?.decisionAt || a.outgoingQuote?.sentAt || 0);
    });
  ...
  function renderWorkspace() {
    ...
    const audit = decision ? `<section class="approval-panel"><div class="approval-panel-head"><div><h3>Immutable customer decision</h3><p><span class="approval-status ${escapeHtml(status)}">${escapeHtml(status.replaceAll('_', ' '))}</span></p></div><strong>${dateTime(decision.decisionAt)}</strong></div>${decision.changeRequestMessage ? `<div class="approval-change-request"><span>Customer requested changes</span><p>${escapeHtml(decision.changeRequestMessage)}</p></div>` : ''}<details class="workflow-audit-details"><summary>Audit Evidence</summary><div class="approval-audit"><div><strong>${escapeHtml(decision.typedName)}</strong><span>Customer-entered name</span></div><div><strong>${decision.termsAccepted ? 'Confirmed' : 'Not applicable'}</strong><span>Terms agreement</span></div><div><strong>${escapeHtml(decision.termsHash)}</strong><span>Terms SHA-256</span></div><div><strong>${escapeHtml(decision.quoteSnapshotHash)}</strong><span>Quote snapshot SHA-256</span></div>${decision.ipAddress !== undefined ? `<div><strong>${escapeHtml(decision.ipAddress || 'Unavailable')}</strong><span>Request IP (admin/manager)</span></div><div><strong>${escapeHtml(decision.userAgent || 'Unavailable')}</strong><span>Truncated user agent</span></div>` : ''}</div><h4>Consent statement shown</h4><p class="approval-message">${escapeHtml(decision.consentText)}</p></details></section>` : '<section class="approval-panel"><h3>Awaiting customer decision</h3><p>The current secure quote is still pending.</p></section>';
